...
sudo curl -I http://localhost/info.php
...
sudo mysql -u root -p -e "CREATE USER 'wp_user'@'localhost' IDENTIFIED BY 'ubuntu';"

# Otorga todos los permisos necesarios al usuario creado en la base de datos de WordPress.
sudo mysql -u root -p -e "GRANT ALL ON wordpress_db.* TO 'wp_user'@'localhost' IDENTIFIED BY 'ubuntu';"
...
sudo curl -I http://localhost/wordpress